...
def read_main_config() -> dict:
    ...
    try:
        ...
        # main.py (v3.1) has no QUIT_KEY constant — it calls keyboard.wait('esc')
        # directly in the __main__ block. calibrate_rois() also calls
        # keyboard.wait('q') to abort calibration, so take the LAST match in
        # the file (the real run-loop quit key), not the first.
        matches = re.findall(r"keyboard\.wait\(['\"](\w+)['\"]\)", src)
        if matches: cfg["quit_key"] = matches[-1]
    except OSError:
        pass
    return cfg
...
class MacroHub(tk.Tk):
    def __init__(self):
        ...
        if not os.path.exists(MAIN_SCRIPT):
            messagebox.showwarning(
                "main.py not found",
                f"Couldn't find main.py next to this hub:\n{MAIN_SCRIPT}"
            )
    ...
    def _launch(self, args: list[str], mode: str):
        if self.proc is not None:
            ...
            return
        if not os.path.exists(MAIN_SCRIPT):
            messagebox.showerror("main.py not found", f"Expected at:\n{MAIN_SCRIPT}")
            return
    ...
    def _route_log_line(self, line: str):
        ...
        if " ERROR" in low or "TRACEBACK" in low or "ERROR:" in low:
            ...
            tag = "warning"
        elif "ACCEPTED" in low and "5/5" in low:
            tag = "success"
        else:
            tag = "info"
        self._log(line, tag)
